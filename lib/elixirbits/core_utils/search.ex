defmodule Elixirbits.CoreUtils.Search do
  ...
  def search(opts) do
    ...
    |> apply_load(load, resource)
    |> execute(pagination, read_opts)
  end
  ...
  defp build_single_scalar_expr(field, rel, value, resource, ctx) do
    ...
    if exact_type?(resource, field) do
      converted = convert_value(resource, field, value, ctx.user_timezone)
  ...
  defp build_range_expr(field, rel, values, resource, ctx, negate?) do
    case values do
      [v1, v2] ->
        r = field_ref(rel, field)
        low = convert_value(resource, field, v1, ctx.user_timezone)
      ...
      [v] ->
        ...
        converted =
          if keyword in ["before", "before_equal"] do
            ...
            convert_value(resource, field, v, ctx.user_timezone)
          end
  ...
  defp build_exact_single_expr(field, rel, values, resource, ctx) do
    case values do
      ...
      [v] ->
        r = field_ref(rel, field)
        converted = convert_value(resource, field, v, ctx.user_timezone)
        Ash.Expr.expr(^r == ^converted)
  ...
  defp build_not_match(:exact, field, rel, values, resource, ctx) do
    r = field_ref(rel, field)
    converted = Enum.map(values, &convert_value(resource, field, &1, ctx.user_timezone))
  ...
  defp build_fields_op_expr(op, value, ctx) do
    ...
    if not valid? do
      ...
      convert = &convert_value(ctx.resource, first_field, &1, ctx.user_timezone)
  ...
  defp apply_load(query, false, _resource), do: query
  defp apply_load(query, [], _resource), do: query

  defp apply_load(query, true, resource) do
    rels = resource |> Ash.Resource.Info.public_relationships() |> Enum.map(& &1.name)
    Ash.Query.load(query, rels)
  end

  defp apply_load(query, load, _resource), do: Ash.Query.load(query, load)

  defp execute(query, %{page: page, per_page: per_page}, read_opts)
  ...
  defp execute(query, _pagination, read_opts) do
    entries = Ash.read!(query, read_opts)
  ...
  defp normalize_type(type) when is_atom(type) do
    cond do
      ...
      type == Ash.Type.Time -> :time
      type == Ash.Type.UUID -> :uuid
      type == Ash.Type.UUIDv7 -> :uuid
      type == Ash.Type.Atom -> :atom
      type == Ash.Type.String -> :string
      type == Ash.Type.CiString -> :string
      type == Ash.Type.Binary -> :binary
      type == Ash.Type.Map -> :map
      enum_type?(type) -> :atom
      true -> infer_from_storage(type)
    end
  end
  ...
  defp infer_from_storage(type) do
    if Code.ensure_loaded?(type) and function_exported?(type, :storage_type, 1) do
  ...
  defp exact_type?(resource, field) do
    case field_type(resource, field) do
      type when type in @exact_types -> true
      _ -> false
    end
  end
  ...
  defp convert_value(_resource, _field, nil, _tz), do: nil

  defp convert_value(resource, field, value, tz) do
    case Ash.Resource.Info.attribute(resource, field) do
      nil ->
        value

      attr ->
        case normalize_type(attr.type) do
          :utc_datetime -> convert_datetime(:utc_datetime, value, tz)
          :naive_datetime -> convert_datetime(:naive_datetime, value, tz)
          :date -> convert_date(value)
          _ -> cast_via_ash(attr, value)
        end
    end
  end

  defp cast_via_ash(attr, value) do
  ...
  defp convert_datetime(target, value, tz) do
    cond do
      is_struct(value, DateTime) ->
        dt = DateTime.shift_zone!(value, "Etc/UTC")
        if target == :naive_datetime, do: DateTime.to_naive(dt), else: dt

      is_struct(value, NaiveDateTime) ->
        if target == :naive_datetime do
          value
        else
          value
      ...
      is_binary(value) ->
        parsed = parse_binary_datetime(value, tz)

        cond do
          target == :naive_datetime and is_struct(parsed, DateTime) -> DateTime.to_naive(parsed)
          true -> parsed
        end
  ...
  defp parse_binary_datetime(value, tz) do
  ...
  defp convert_date(v), do: v

  defp handle_datetime_range_end(resource, field, value, tz) do
    converted = convert_value(resource, field, value, tz)

    case {field_type(resource, field), converted} do
      {:utc_datetime, %DateTime{} = dt} -> %{dt | second: 59, microsecond: {0, 0}}
      {:naive_datetime, %NaiveDateTime{} = ndt} -> %{ndt | second: 59, microsecond: {0, 0}}
      _ -> converted
    end
  end
