...
- This project is heavily inspired by the references in `../sig-portal/`. Treat them as an idea reference only: you may mirror their concepts, flows, UX decisions, and logic, because that is how I like things done -- but never copy code from them. They are **not** "reference implementations" for the reference-fidelity rules in `## Strict Guidelines`; those rules apply to code inside this repository, or to a reference I explicitly hand you for that purpose.
...
- **Never** add, remove, or upgrade a dependency on your own. If a task appears to require one, stop, propose it in your response (which package, why, and what existing stdlib/dependency option falls short), and wait for my approval.
...
- Never add new comments (this includes docs such as `@doc`/`@moduledoc` and other doc attributes) unless instructed to. However, you are required to modify existing comments when necessary -- for example, to keep them accurate after adding or removing functionality -- following a similar writing style. Typespecs (`@spec`) are code, not comments: add or omit them by matching what the surrounding modules already do.
